...
export const fetchGames = createAsyncThunk('games/fetchGames', ({ genre, platform, sort }: { genre: string | null, platform: string | null, sort: string | null }) => {
  ...
  // ! Закомментированный блок ниже это бэкенд для хостинга статики и API для инкапсуляции внешних запросов на Node.JS.
  // ! Закомментировал данный блок, поскольку сервис Render может приостанавливать работу неактивных проектов, поэтому
  // ! для избежания ситуации при которой игры не будут получены, я закомментировал данный блок.
  // ! При необходимости его можно раскомментировать и затестить бэкенд сервис, который написал.
  ...
  // basicUrl += sort ? `&sort=${sort}` : ''

  // ! Закомментировать блок ниже в случае тестирования бэкенда:
...
export const fetchGame = createAsyncThunk('games/fetchGame', (id: number) => {
  ...
  // ! Закомментировать переменную ниже в случае тестирования бэкенда:
